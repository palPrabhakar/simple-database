...
TEST(OperatorTest, JsonReader) {
  // GTEST_SKIP();
  sdb::JsonReader read_op("large_table");
  read_op.Execute();
...
TEST(OperatorTest, SimpleSelect) {
...
TEST(OperatorTest, SelectWithWhere) {
...
TEST(OperatorTest, CreatePipeline) {
  // empty table
  auto operators = sdb::ParseInputQuery(
...
TEST(OperatorTest, InsertPipeline) {
  auto operators = sdb::ParseInputQuery("insert test_table values 42 2.5 lol");
...
TEST(OperatorTest, JoinPipeline) {
  auto operators = sdb::ParseInputQuery(
...
TEST(OperatorTest, JoinPipelineWithWhere) {
  auto operators = sdb::ParseInputQuery(
      "select * from jl_table join jr_table on ( "
...
TEST(OperatorTest, UpdatePipeline) {
  auto operators =
...
TEST(OperatorTest, UpdateWithWhere) {
  auto operators = sdb::ParseInputQuery(
